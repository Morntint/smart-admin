...
import { getTokenExpireTime } from '@/utils/http/jwt'
...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    // 访问令牌过期时间（毫秒级时间戳，0 表示未知）
    const accessTokenExpire = ref(0)
    ...
    const setToken = (newAccessToken: string, newRefreshToken?: string) => {
      ...
      // 从 JWT 中解析过期时间，供 Token 自动刷新逻辑使用
      accessTokenExpire.value = getTokenExpireTime(newAccessToken)
      if (newRefreshToken) {
