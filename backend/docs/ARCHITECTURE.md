...
┌──────────────────────────────────────────────────────────────┐
│  Middleware 链                                                 │
│  - Trace（全局，生成/透传 request_id）                          │
│  - Metrics（全局，请求计时与 Prometheus 指标上报）              │
│  - StaticFile（全局，拦截 /. 隐藏文件）                         │
│  - Cors（全局，跨域 + 预检）                                    │
│  - AuthMiddleware（admin，鉴权 + 权限 + Token 版本校验）        │
│  - RateLimit（admin，按 #[RateLimit] 注解限流）                 │
│  - Idempotent（admin，按 #[Idempotent] 注解防重复提交）         │
...
| 接口限流计数 | `rate_limit:{scope}\|{dimension}` | 注解 window |
| 登录失败计数 | `rate_limit:login_fail:{username}` | 15 min |

> 用户信息缓存 `auth_user_{uid}` 含 `token_version` 字段，用于 JWT 主动失效校验。
